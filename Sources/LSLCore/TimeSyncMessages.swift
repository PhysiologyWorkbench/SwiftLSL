...
public enum TimeSyncMessage {
    /// ```
    /// LSL:timedata\r\n
    /// <wave id> <t0>\r\n
    /// ```
    /// with 16 significant digits, as `liblsl` writes it.
    public static func probe(waveID: Int32, t0: Double) -> Data {
        Data("LSL:timedata\r\n\(waveID) \(format(t0))\r\n".utf8)
    }
    ...
    public static func reply(waveID: Int32, t0: Double, t1: Double, t2: Double) -> Data {
        Data(" \(waveID) \(format(t0)) \(format(t1)) \(format(t2))".utf8)
    }

    public struct Reply: Sendable, Hashable {
        public let waveID: Int32
    ...
    public static func parseReply(_ datagram: Data) throws -> Reply {
    ...
    /// (SCOPE.md §2.5).
    public static func measure(_ reply: Reply, t3: Double) -> (rtt: Double, offset: Double) {
