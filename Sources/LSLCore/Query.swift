...
package enum Query {
    ...
    package static func session(_ sessionID: String, and predicate: String? = nil) -> String {
        let base = "session_id='\(escape(sessionID))'"
        guard let predicate, !predicate.isEmpty else { return base }
        return "\(base) and \(predicate)"
    }

    package static func property(_ name: String, equals value: String) -> String {
        "\(name)='\(escape(value))'"
    }
    ...
    package static func recovery(for info: StreamInfo) -> String {
