...
Supports-Subnormals: <1 for float32/double64, else 0>\r\n
...
`Supports-Subnormals` is `format_subnormal[channel_format]`, not a constant 1
(`src/data_receiver.cpp:181-182`, `src/sample.h:26-29`) — it is 1 only for `float32` and
`double64`. Declaring 1 unconditionally is harmless, because the outlet computes
`suppress = format_subnormal[format] && !client_supports`, but it is not what the
reference sends.

`Endian-Performance` is a byte-swapping throughput figure in the *client's* units, and the
outlet converts on the inlet's behalf only if its own measurement beats it
(`src/tcp_server.cpp:655-663`). It is therefore a policy knob, not a fact: declaring 0
means "you convert if you can", declaring a large value means "send me native order and
I will convert". `liblsl` benchmarks at connect time; either choice interoperates.
...
- Header keys are lower-cased before matching, and anything after `;` is stripped as a
  comment (`src/data_receiver.cpp:219-226`). The *values* are lower-cased too, which is
  why the UID comparison is effectively case-insensitive.
...
- The outlet reads the client's version preference from a header named
  `Protocol-Version`, while the inlet writes `Data-Protocol-Version`
  (`src/tcp_server.cpp:626`). The two never meet, so the version actually negotiated is
  the one on the request line. Harmless, but do not "fix" it.
...
`int16`, `1` for `int8`, `2147483649` for `int64`; `int32`, `int16` and `int8` are taken
modulo `T::max` **before** the sign is applied, while `int64` is generated by a separate
branch that takes no modulo at all (`src/sample.cpp:390-399`); `string` channels ignore the
offset entirely and use `to_string((k+10) * (k%2==0 ? 1 : -1))`.
...
back as `stod(...) * 100`, truncated.

Serialisation details, confirmed against liblsl 1.17.7 output:

- Every `double` field is written by a locale-independent helper equivalent to
  `printf("%#.16g")` — 16 significant digits with trailing zeros kept
  (`src/util/cast.cpp:9-14`). So a 512 Hz stream advertises
  `<nominal_srate>512.0000000000000</nominal_srate>` and every 1.10 outlet advertises
  `<version>1.100000000000000</version>`.
- `pugixml` writes an XML declaration, indents with tabs and separates lines with `\n`.
  A field an outlet has set to the empty string is `<uid></uid>` (it holds an empty text
  node), whereas an element with no child node at all is `<desc />`. A shortinfo therefore
  ends `\t<desc />\n</info>\n`.
- **An outlet advertises `<v4address></v4address>` empty.** The inlet's own record of the
  address comes from the source of the discovery reply (§2.1), never from the XML; a
  resolver that trusts the XML alone has no address to connect to.
...
Signatures only. Deviations made during implementation are listed at the end of the
section.
